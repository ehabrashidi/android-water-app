<?php
include("PDOConnection-fahs.php");
...
{
	$query = "INSERT INTO fahs(fahs_t, E_number, date_of_day) VALUES(?, ?,now())";
...
{
	$query = "SELECT * FROM fahs WHERE fahs_t = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->execute();
...
{
	$query = "SELECT * FROM fahs WHERE fahs_t = ? AND E_number = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->bindParam(2, $pwd);
	$stmt->execute();
