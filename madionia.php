<?php
include("PDOConnection-madionia.php");
...
{
	$query = "INSERT INTO madionia(E_number, madionia_val) VALUES(?, ? )";
...
{
	$query = "SELECT * FROM madionia WHERE E_number = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->execute();
...
{
	$query = "SELECT * FROM madionia WHERE E_number = ? AND E_number = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->bindParam(2, $pwd);
	$stmt->execute();
