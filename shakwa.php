<?php
include("PDOConnection-shakwa.php");
...
{
	$query = "INSERT INTO shkawy(shakwa_t, E_number,date_of_day) VALUES(?, ?,now())";
...
{
	$query = "SELECT * FROM shkawy WHERE shakwa_t = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->execute();
...
{
	$query = "SELECT * FROM shkawy WHERE shakwa_t = ? AND E_number = ?";
	$stmt = $cnn->prepare($query);
	$stmt->bindParam(1, $username);
	$stmt->bindParam(2, $pwd);
	$stmt->execute();
